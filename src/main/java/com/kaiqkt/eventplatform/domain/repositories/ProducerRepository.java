...
public interface ProducerRepository extends JpaRepository<Producer, String> {
    @Query("SELECT p FROM Producer p JOIN p.versions v WHERE p.service = :service AND p.action = :action " +
            "AND v.value = :versionValue")
    Optional<Producer> find(
            @Param("service") String service,
            @Param("action") String action,
            @Param("versionValue") Integer versionValue
    );
    Optional<Producer> findByServiceAndAction(String service, String action);
    @Query("SELECT p FROM Producer p WHERE (:service IS NULL OR p.service = :service) AND " +
